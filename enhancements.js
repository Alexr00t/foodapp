...
function renderAutosave(){if(!autoBox)return;autoBox.querySelector('#auto-mode').textContent=AUTOSAVE?'Activ':'Oprit';const last=autoLog[0];autoBox.querySelector('#auto-last').textContent=last?'Ultima salvare: '+relative(last.time)+(dirHandle?' • folder selectat':''):'Încă nu au avut loc salvări în această sesiune.';const list=autoBox.querySelector('#auto-log');list.innerHTML='';autoLog.forEach(entry=>{const li=document.createElement('li');li.textContent=relative(entry.time)+' · '+entry.kind;list.appendChild(li);});}
...
function rebuildIndex(){clearTimeout(buildTimer);buildTimer=setTimeout(()=>{index=new Map();getAllProducts().forEach(p=>{index.set(p.name,{name:normalize(p.name),tags:(p.tags||[]).map(normalize)});});},80);}
